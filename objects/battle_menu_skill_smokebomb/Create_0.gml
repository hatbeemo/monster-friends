...
ap_cost=Flag_Get(FLAG_TYPE.TEMP,FLAG_TEMP.WHIMSIE_BOMB_FUSE+add_passive,1)-1;
...
name+="Smoke Bomb{color_text `white`}[{color_text `specaqua`}"+string(ap_cost)+"{color_text `white`}]"
info="Whimsie throws a bomb that will&give {color_text `specaqua2`}"+string(fuse*2)+" BLOCK{color_text `white`} upon detonation.&The bomb will have a Fuse of "+string(fuse)+"."
...
power_info="{color_text `specgold`}PASSIVE: Fast Arms&{color_text `white`}Throw an extra bomb in the&same turn! {color_text `specaqua2`}("+string(fuse*2)+" BLOCK){color_text `white`}"
damage=2;
deals_dmg=1;
target="ALL";
_done=false;
